...
void sha_256_write(struct SHA256 *sha_256, const void *data, size_t len)
{
	...
	while (len > 0) {
		...
		_memcpy(sha_256->chunk_pos, p, consumed_len);
...
inline int cmp_hash(void *hash1, void *hash2)
{
	uint32_t *one = (uint32_t *)hash1;
	uint32_t *two = (uint32_t *)hash2;
	return
		one[0] == two[0] &&
		one[1] == two[1] &&
		one[2] == two[2] &&
